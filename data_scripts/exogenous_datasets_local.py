...
class ExogenousDatasetLocal:
    def __init__(self,column_metadata_file,datainput_file,max_hist,k_week_ahead,training_end_date,with_endog_features=False,
                input_feature_prefix="input_feature",num_test_weeks=3,region_shuffle=True,DEBUG=False,endog_scaler=None):
        """
            ...
            @param DEBUG: This is a flag which is set to run this script in DEBUG model. Essentially, this script turns off data scaling so data can be visually analyzed for correctness in terms of temporal alignment etc.
        """
        ...
        self.DEBUG=DEBUG
        self.endog_scaler = endog_scaler
        ...
        self.num_regions = self.data[self.region_col].unique().shape[0]  #Store this for use later on. This should always be 1 in the case of this file.
    ...
    def read_metadata(self):
        ...
        self.current_region=obj['region']
    
    def read_all_exogenous_data(self):
        all_exog_data = pd.read_csv(self.data_input_file)
        self.data = all_exog_data #Temporarily assigning self.data to be all_exog_data.
        #To maintain uniformity in one-hot encodings, we create one-hot encodings before we filter by region.
        #Add Region One-Hot Encoding.
        self.create_categorical_features()
        
        #Use data that only belongs to a particular region
        all_exog_data = all_exog_data[all_exog_data[self.region_col]==self.current_region]

        all_exog_data.reset_index(drop=True,inplace=True)
    ...
    def fit_scalers_per_region_per_col(self,col,region,transformation):
        ...
        flag=False #Flag to differentiate between fitting a scaler for regular columns and for just storing a previously fitted scaler in the case of `endog-scaler`
        ...
        # endogeneous data column needs to have the same name in both datasets 
        elif transformation=='endog-scaler' and self.endog_scaler is not None:
            flag=True
            _scaler = self.endog_scaler[region][self.target_col] 
        
        if not flag: #If Flag is not true, fit the scaler. Else, just store it.
    ...
    def _fit_transform_col(self,region,col,transformation='standard-scaler',NONZERO_CONST=1e-1,scaler=None):
        ...
        # endogeneous data column needs to have the same name in both datasets 
            ...
            _scaler = self.endog_scaler[region][col]            

        if transformation=='endog-scaler':
            per_region_data_train.loc[:] = _scaler.transform(per_region_data_train.values[:,None]).ravel()
        else:
